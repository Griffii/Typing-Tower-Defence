# res://scripts/game/towers/lightning_tower.gd
extends Node2D
...
func _get_first_target() -> Node2D:
	...
	match targeting:
		"nearest":
			return _get_nearest_target_to_position(_get_fire_origin(), targets_in_range, {})

		_:
			return _get_nearest_target_to_position(_get_fire_origin(), targets_in_range, {})


func _fire_chain_lightning_sequence(first_target: Node2D) -> void:
	if is_firing_chain:
		return
	...
	while hits_done < max_chain_targets:
		...
		if hits_done >= max_chain_targets:
			break
...
func _spawn_lightning_projectile_on_enemy(target) -> void:
	...
	var projectile: Node2D = TOWER_PROJECTILE_SCENE.instantiate() as Node2D
	projectile_container.add_child(projectile)

	projectile.global_position = target.global_position

	if projectile.has_method("fire_on_target"):
		projectile.fire_on_target(target)
...
func _find_next_chain_target_from_position(origin: Vector2, attacked_enemy_ids: Dictionary) -> Node2D:
	var candidates: Array[Node2D] = _get_enemies_in_chain_area(origin, attacked_enemy_ids)

	if candidates.is_empty():
		return null

	return _get_nearest_target_to_position(origin, candidates, attacked_enemy_ids)


func _get_enemies_in_chain_area(origin: Vector2, attacked_enemy_ids: Dictionary) -> Array[Node2D]:
	...
	for enemy_node in get_tree().get_nodes_in_group("enemies"):
		...
		if attacked_enemy_ids.has(enemy_id):
			continue

		if origin.distance_to(enemy.global_position) > chain_range:
			continue
...
func _get_nearest_target_to_position(origin: Vector2, candidates: Array[Node2D], ignored_enemy_ids: Dictionary) -> Node2D:
	...
	for enemy in candidates:
		...
		var enemy_id: int = enemy.get_instance_id()
		if ignored_enemy_ids.has(enemy_id):
			continue
